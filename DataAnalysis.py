...
if __name__ == "__main__":
    ...
    # Loading the dataset
    # JSON files
    train_json = os.path.join("bdd100k_labels_release", "bdd100k_labels_images_train.json")
    ...
    train_stats, train_bboxes = run_dataset_analysis("Train", train_data, train_img_root, excluded, "plots/train")
    val_stats, val_bboxes = run_dataset_analysis("Val", val_data, val_img_root, excluded, "plots/val")

    print("Generating Summaries")
    # Summaries
    generate_category_summary(train_stats['category'], val_stats['category'], excluded)
    generate_attribute_summary(train_stats["weather"], val_stats["weather"], "weather", excluded)
    generate_attribute_summary(train_stats["scene"], val_stats["scene"], "scene", excluded)
    generate_attribute_summary(train_stats["timeofday"], val_stats["timeofday"], "timeofday", excluded)
